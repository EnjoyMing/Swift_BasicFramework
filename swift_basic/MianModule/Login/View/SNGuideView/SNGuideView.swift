...
class SNGuideView: UIView {

    @objc public static func showGuideView() -> Void {
        // gif和jpg类型的资源数组
        let imageGifArray = ["launchImg","launchImg", "launchImg", "launchImg","launchImg", "launchImg"]
        let guideView = SNGuideView.init(images: imageGifArray, loginRegistCompletion: {
            //登录注册按钮
        }) {
            UserDefaults.standard.set(true, forKey: SaveKey.FirstOpenApp)
            UserDefaults.standard.synchronize()
            SNInitObject.share.setRootVC()
        }
        var window:UIWindow!
        if #available(iOS 13, *) {
    ...
    /// 指示器
    public lazy var pageControl: SNPageControl = {
        var pageControl = SNPageControl()
...
//        btn.backgroundColor = UIColor.init(red: 177.0/255.0, green: 126.0/255.0, blue: 71.0/255.0, alpha: 1.0)
        btn.setBackgroundImage(self.imageFromBundle(name:"start_btn_bg"), for: .normal)
    ...
    private func setupSubviews(frame: CGRect) {
        ...
        for index in 0..<(imageArray?.count ?? 1) {
            ...
            var view: UIView
            let type = SNGifImageOperation.checkDataType(data: data)
            if type == DataType.gif {   // gif
                view = SNGifImageOperation.init(frame: imageFrame, gifData: data!)
            } else {                    // 其它图片
...
extension SNGuideView: UIScrollViewDelegate {
